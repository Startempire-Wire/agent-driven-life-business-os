...
- **Composable AI workforce catalogue added:** derived proposed Chief-of-Staff, manager, flash/campaign/staff worker profiles, generic task packs, CRIST depth, state/lifetime, scheduling, evidence and client roster semantics from current Focusa/UIAI capabilities plus the operator’s administrative-duty example. Added verbatim Focusa tool/spec/code quotations and recorded two live tool-descriptor drift findings instead of copying incorrect metadata.
