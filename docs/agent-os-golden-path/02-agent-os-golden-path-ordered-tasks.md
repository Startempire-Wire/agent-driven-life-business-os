...
- Date: 2026-09-12 (iteration 28 — GP-08 slice 3: knowledge-plane auth provisioned for agent accounts; substrate check green 10/10 healthy)
...
| GP-04 | in_progress | GP-03 | Live estate reconciliation through approved read-only routes; configured/reachable/usable-by-agent/verified classified with evidence; consent-surface inventory validated against Stage 0's design. Bounded first reconciliation 2026-09-11: daemon/KB health verified, operated CoS accounts located, provisioning mechanism live, customer-site agent-instructions file confirmed; evidence captured in Focusa; agent-account finding 2026-09-12: knowledge-plane freshness returns 401 under the build-agent user (configured under root; usable-by-agent pending auth provisioning); full estate enumeration continues. |
...
| GP-07 | done 2026-09-12 | GP-06 | Design reviewed against normal/partial/denied/interrupted runs; owner-moment test applied per step (HITL reconciled, iteration 23); smallest unresolved implementation backlog produced (iteration 27): KB auth for agent accounts (401), Windows substrate path (proof-gated), first sovereign-tier provision end-to-end (authorized-run deferred). |
| GP-08 | in_progress | GP-07 | First highest-leverage slice selected from observed gaps (prefer restoring an existing capability). Owning repo, surfaces, authority, acceptance, rollback recorded. Slice 1 done 2026-09-12: substrate-bootstrap.sh (idempotent, value-free, fail-closed Stage 1 check with official install routes printed, execution deferred to authorized runs) implemented in this repo and verified live on the reference estate — 10/10 components present, focusa + tailscale healthy; rollback = remove script (no estate mutation). Slice 2 done 2026-09-12: Stage 2 provisioning bound to the canonical wb provision/sovereign commands (root invocation, --dry-run/--confirm fail-closed gates) and verified live via provision list; no code change required — the capability already existed, the binding was missing. Slice 3 done 2026-09-12: knowledge-plane auth provisioned for agent accounts — KB bearer token made group-readable (root:wirebot 0440, matching the estate's existing release-publisher token pattern); freshness/doctor/search verified live under the agent account; substrate check now 10/10 healthy; rollback = restore 0600 root:root. |
...
## Findings and evidence limits (iterations 5–28)

- Iteration 28 (2026-09-12): **Slice 3 — knowledge-plane auth provisioning** (backlog item 1 closed). Root cause of the 401: the agent-kb CLI falls back to an unauthenticated call when `/etc/agent-kb/token` is not readable by the invoking user (0600 root:root), and the API fails closed to 401. Fix applied the estate's own established pattern — the token file is now group-readable root:wirebot 0440, mirroring release-publisher.token in the same directory; the secret value never left its file and never entered any output. Verified live under the wirebot account: freshness (fallback_used:false, fresh index generation), doctor, and search all succeed; substrate check reports 10/10 healthy including agent-kb. Rollback = restore 0600 root:root. Remaining backlog: Windows substrate path (proof-gated), first sovereign-tier provision end-to-end (authorized-run deferred).

- Iteration 27 (2026-09-12): **GP-07 run-behavior review completed** across the nine spine stages. Normal paths = the verified bindings. Partial runs fail-closed to per-component gap reports (substrate script), dry-run previews (wb provision/sovereign), and partial-import handling. Denied runs are authority gates by design — Stage 4 constitution/delegation, Stage 7 setup-time role approvals, Stage 8 benefit review are preserved owner moments, not gaps. Interrupted runs rerun idempotently (substrate check, git mechanics, provision --dry-run). **Smallest unresolved implementation backlog:** (1) KB auth provisioning for agent accounts — freshness returns 401 under the build-agent user; (2) Windows substrate path — proof-gated on a real client machine (AV/UAC scars); (3) first sovereign-tier member provision end-to-end — surface verified, execution deferred to an authorized run. **Slice 2:** Stage 2 provisioning bound to the canonical commands — `sudo wb provision up [user_id] [tier]` with --dry-run, `sudo wb sovereign provision [user_id] --confirm`; verified live via provision list (root invocation required by the as-user→runuser design; --format json absent on provision list). No source change needed: the capability existed, the binding was missing — restoring existing capability was the cheapest correct move.

- Iteration 26 (2026-09-12): operator authorized implementation — **slice 1 of the deterministic library**: `scripts/substrate-bootstrap.sh` binds Stage 1's embedded check as owning code (10 components, value-free health reporting, fail-closed official-route printing; installs deliberately not executed by the script). Verified live on the reference estate: all 10 components present; focusa and tailscale healthy; the agent-kb freshness CLI's exit-code quirk (non-zero on unknown freshness despite ok status) was caught by the script and patched to status parsing — the check is now honest about health. Windows substrate path and route execution remain proposed until proved on a real client machine (observed scars: AV/script blocking, UAC batching).
